  ...
  var cl = {
    ...
    renderUI: function () {
      ...
      for (var i = 0; i < s.stock_array.length; i++) {
        ...
        var el = $(
          sprintf(
            "<div class='bin-outer'><div class='bin-label'>%s</div><div class='bin-inner' style='height: %s%%'></div></div>",
            s.stock_array[i].size,
            display_size
          )
        );
        $("#floor").append(el);
        ...
        for (var j = 0; j < cuts.length; j++) {
          ...
          var cut_el = $(
            sprintf("<div class='parcel' style='height: %spx; line-height: %spx'>%s</div>", cut_size, cut_size, cuts[j])
          );
        ...
        if (remained > 0) {
          var cut_size = Math.floor((inner_el.height() - 1) * (remained / s.stock_array[i].size)) - 1;
          var cut_el = $(
            sprintf("<div style='height: %spx; line-height: %spx'>%s</div>", cut_size, cut_size, remained)
          );
          inner_el.append(cut_el);
        }
      }
    ...
    onCutAdd: function (e) {
      ...
      if (isNumber(len) && isNumber(qty)) {
        s.cut_list.push([qty * 1, len * 1]); // [#, len]
        CutList.renderUI();
      }
      s.cut_length.val("");
    ...
    sortStockDisplay: function () {
      s.stock_array.sort(function (a, b) {
        var a_remaining = a.size - arraySum(a.cuts, s.kerf);
        var b_remaining = b.size - arraySum(b.cuts, s.kerf);
        if (a.size == b.size) return a_remaining - b_remaining;
    ...
    optimize: function () {
      ...
      for (var i = 0; i < s.cut_array.length; i++) {
        var index = 0;
        if (s.stock_array.length == 0) this.addOverflow();
        ...
        while (s.stock_array[index].size - arraySum(s.stock_array[index].cuts, s.kerf) < s.cut_array[i]) {
          index++;
          if (index >= s.stock_array.length) this.addOverflow();
        }
